...
def buyandhold(
    ...
    try:
        ...
        # _end는 기본값은 시계열인 경우는 아무 행동도 하지 않는다
        ...
        print(f"시작시관과 종료시간의 포맷은 YYYY-mm-dd 형식입니다")
        return ""
    ...
    print(f"{start.strftime('%Y-%m-%d')}부터 {end.strftime('%Y-%m-%d')}까지 buyandhold의 수익률은 {result.iloc[-1, 2]}입니다")
